...
11. **Installer bundles IPFS Kubo + Nginx** (added 2026-04-29 in v0.4.0). Vendored under `src/src/vendor/{ipfs,nginx}/` (gitignored, fetched by [scripts/fetch-vendor-binaries.mjs](src/src/scripts/fetch-vendor-binaries.mjs) on `build:win`/`build:unpack`). Lands at `resources/extras/{ipfs,nginx}/` in the installed app. Currently dormant — IPFS allocation feature and ExecEngine HTTP/Nginx layer ship in v2. **Bootstrap-fleet contract for v2:** [src/shared/ipfs-bootstrap.ts](src/src/src/shared/ipfs-bootstrap.ts) holds the SCS-Polaris bootstrap multiaddrs in `/dnsaddr/` form (rotation-friendly, dual-stack via TXT records). When v2's daemon-start code lands, import `buildKuboBootstrapList()` and write into the Kubo config `Bootstrap` field at `%IPFS_PATH%\config` on first run. **Windows-only project — Kubo defaults to `%LOCALAPPDATA%\ipfs\config` if `IPFS_PATH` is unset; v2 should set IPFS_PATH explicitly to a subdir of `app.getPath('userData')` so the per-app store doesn't collide with a standalone Kubo install.** The list ships with 10 placeholder PeerIDs marked via `PLACEHOLDER_PEER_ID_MARKER`; `hasPlaceholderPeerIds()` detects that state — daemon-start MUST refuse to start while it returns true (better to surface "fleet not provisioned" than dial 10 timeouts).
