...
function createWindow() {
  ...
  ipcMain.on("LLM_Model_Update_Default", (event, LLM_Model_ID) => {
    db.run("UPDATE Models SET isDefault = 'N'", function(err) {
      if (err) {
        console.error("Error setting all isDefault to 'no':", err);
        return;
      }
  
      db.run("UPDATE Models SET isDefault = 'Y' WHERE ModelID = ?", [LLM_Model_ID], function(err) {
        if (err) {
