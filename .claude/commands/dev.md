...
2. From `d:/Client-Side_Project/ShortCut_Studio/src/src/`, run `npm run dev` in the background. **IMPORTANT:** `unset ELECTRON_RUN_AS_NODE` first — Claude Code's bash sets it by default and Electron will crash on `app.on()` if it stays set.
