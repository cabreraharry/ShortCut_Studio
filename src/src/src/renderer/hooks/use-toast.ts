...
export function toast(props: Toast): { id: string; dismiss: () => void; update: (p: Partial<ToasterToast>) => void } {
  const id = genId()
  const update = (p: Partial<ToasterToast>): void =>
...
export function useToast(): State & { toast: typeof toast; dismiss: (id?: string) => void } {
